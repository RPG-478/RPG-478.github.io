...
import { History, Layout, Plus, Trash2, Github, Server, Cloud, GitBranch, Database, UserCheck, Box, Workflow, FileEdit, X, BrainCircuit, Calendar, Clock, Milestone, HelpCircle, Sparkles, Terminal, Code2, Users, MessageSquare, Bug } from 'lucide-react';
      ...
      `}>
        ...
        <div className={`p-4 border-t space-y-1 ${
          ...
          <button 
            ...
            className={`flex items-center gap-3 px-3 py-2 text-sm transition-colors ${
              isDev ? 'text-slate-500 hover:text-red-400' : 'text-slate-500 hover:text-red-500'
            }`}
          >
            <Bug className="w-4 h-4" />
            {isDev ? 'Bug report' : 'バグ報告'}
          </button>
          <button 
            onClick={onShowHelp}
